...
export class Game2 extends Scene {
  constructor(private currentIndex:number, private combo:Direction[], private comboImages:Phaser.GameObjects.Image[], private resultText:Phaser.GameObjects.Text, private score:number, private scoreText:Phaser.GameObjects.Text) {
    super("Game2")
    this.currentIndex = 0;
    this.combo = [];
    this.comboImages = [];
  ...
  createCombo(length: number) {
    ...
    for (let i = 0; i < this.combo.length; i++) {
      this.comboImages.push(this.add.image(50 * i + 50, 100, directionText[this.combo[i]]).setOrigin(0.5, 0.5));
    }
  }
  ...
  create() {
    ...
    // Moblie input
    for (let i = 0; i < 4; i++) {
      const button = this.add.sprite(50 + i * 100, screenCenterY+400, `${directionText[i]}-button`).setOrigin(0.5, 0.5)
    ...
    // Result
    this.resultText = this.add.text(screenCenterX, screenCenterY + 200, '').setOrigin(0.5, 0.5);

    // Score
    this.scoreText = this.add.text(screenCenterX, screenCenterY + 300, `Score: ${this.score}`, {
      fontSize: 30
    }).setOrigin(0.5, 0.5)
